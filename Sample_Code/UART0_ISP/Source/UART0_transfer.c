...
xdata volatile uint8_t uart_rcvbuf[UartBufSize+2]; 
xdata volatile uint8_t uart_txbuf[8];
...
{
    uart_txbuf[0]=uart_rcvbuf[0];
	uart_txbuf[1]=uart_rcvbuf[1]+1;
    uart_txbuf[7]=0;
    for(count=0;count<8;count++)
    {
    TI = 0;	
    SBUF = uart_txbuf[count];
    uart_txbuf[7]+=uart_txbuf[count];
    while(TI==0);
    }
}
